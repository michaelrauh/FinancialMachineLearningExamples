...
def graph(balances, balance, horizon, x, start_sim, end_sim, price_change, blacklist_duration, strategy):
    plt.plot(balances)
    plt.title("x=" + str(x) + ", horizon=" + str(horizon) + ", balance=" + str(round(balance, 2)) + " stop at " +
              str(price_change) + " " + "duration " + str(blacklist_duration) + " " + strategy)
