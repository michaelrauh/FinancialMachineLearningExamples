...
def graph(balances, balance, horizon, x, start_sim, end_sim, loss, blacklist_duration, i):
    plt.plot(balances)
    plt.title("x=" + str(x) + ", horizon=" + str(horizon) + ", balance=" + str(round(balance, 2)) + " stop at " +
              str(loss) + "duration " + str(blacklist_duration) + str(i))
    ...
    plt.savefig('output/' + str(start_sim) + "_" + str(end_sim) + "_" + str(x) + "_" + str(horizon) + "_" + str(loss) +
                "_" + str(blacklist_duration) + "_" + str(i) + '.png', bbox_inches='tight')
    plt.gcf().clear()