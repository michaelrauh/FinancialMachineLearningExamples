...
def find_max(date):
    ...
    while(point_map[dates[date_index]][1] <
            point_map[dates[date_index - 1]][1]):
        date_index -= 1
...
def find_min(maximum):
    ...
    while(point_map[dates[date_index]][2] >
            point_map[dates[date_index - 1]][2]):
        date_index -= 1
    return dates[date_index]
...
for stock in stocks:
    ...
    for date in yearLows:
        ...
        coordinates[date].append(slope_max(date, maximum, point_map))

        # Slope from local min #
        coordinates[date].append(slope_min(date, minimum, point_map))
